...
class SupabaseService extends GetxService {
  ...
  Future<void> unassignSupervisorFromDepartment(String supId) async {
    if (!await PermissionService.checkNetwork()) {
      print('No network. Cannot unassign department.');
      throw Exception('No network connection');
    }
    try {
      await _supabase.from('users').update({'department_id': null}).eq('id', supId);
      print('Department unassigned successfully');
    } catch (e) {
      print('Error unassigning department: $e');
      rethrow;
    }
  }
}