...
import 'package:flutter/material.dart';
import 'package:attendance_app/models/attendance_log_models.dart';
import 'package:attendance_app/models/geo_fence_model.dart';
import 'package:attendance_app/services/geo_fence_services.dart';
import 'package:attendance_app/services/permission_service.dart';
import 'package:attendance_app/services/superbase_services.dart';
import 'package:intl/intl.dart';
...
class OfflineLocalStorageService extends GetxService {
  static const String _tableName = 'offline_attendance';
  static const String _geofenceTableName = 'geofences';
  late Database _db;
  final SupabaseService _supabaseService = Get.find<SupabaseService>();
  final GeoFenceService _geoFenceService = Get.find<GeoFenceService>();
  final RxString message = ''.obs;
  ...
  Future<void> init() async {
    ...
    _db = await openDatabase(
      path,
      version: 2, // Updated version to accommodate schema changes
      onCreate: (db, version) async {
        await db.execute('''
          CREATE TABLE $_tableName (
            id TEXT PRIMARY KEY,
        ...
        ''');
        await db.execute('''
          CREATE TABLE $_geofenceTableName (
            id TEXT PRIMARY KEY,
            name TEXT,
            latitude REAL,
            longitude REAL,
    ...
    await _cleanupOldRecords();
    await _refreshGeofences(); // Refresh geofences on init
    _startSyncListener();
    print('OfflineLocalStorageService initialized');
  }

  /// Save geofences to local storage
  Future<void> saveGeofences(List<Geofence> geofences) async {
    try {
      print('Saving ${geofences.length} geofences to local storage');
  ...
  /// Retrieve stored geofences
  Future<List<Geofence>> getStoredGeofences() async {
    try {
      print('Fetching stored geofences');
      final maps = await _db.query(_geofenceTableName);
      final geofences = maps.map((map) => Geofence(
            id: map['id'] as String,
            name: map['name'] as String?,
            latitude: map['latitude'] as double,
            longitude: map['longitude'] as double,
            radius: map['radius'] as int? ?? 100,
          )).toList();
      print('Fetched ${geofences.length} stored geofences');
      return geofences;
    } catch (e) {
      print('Error fetching stored geofences: $e');
      return [];
    }
  }

  /// Refresh geofences from Supabase and store locally
  Future<void> _refreshGeofences() async {
    if (await PermissionService.checkNetwork()) {
      ...
      if (geofences.isNotEmpty) {
        ...
        print('Geofences refreshed from Supabase');
      } else {
        print('No geofences fetched from Supabase');
      }
    } else {
      print('No network, using stored geofences');
      final storedGeofences = await getStoredGeofences();
      if (storedGeofences.isNotEmpty) {
        await _geoFenceService.setupGeofences(storedGeofences);
      }
    }
  }
  ...
  Future<void> syncRecords() async {
    ...
    try {
      final geofences = await _supabaseService.getGeofences();
      print('Geofences for sync: $geofences');
      await saveGeofences(geofences); // Update local geofences during sync
      final unsyncedRecords = await getUnsyncedRecords();
      print('Unsynced records to sync: ${unsyncedRecords.length}');
  ...
  void _startSyncListener() {
    Connectivity().onConnectivityChanged.listen((result) async {
      print('Connectivity changed: $result');
      if (result != ConnectivityResult.none) {
        print('Attempting to sync offline records and refresh geofences...');
        await syncRecords();
